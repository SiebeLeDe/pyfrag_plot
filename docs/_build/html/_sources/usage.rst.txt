...
pyfragplotter is a Python package that can be used to make plots from PyFrag directories containing the .txt (resultsfile) and .in file (inputfile).

To use pyfragplotter, you need to have a config file and call the pyfragplotter.config.initialize_pyfrag_plotter function. The config file should contain the necessary information about the PyFrag directories and the plot settings. The initialize_pyfragplotter function reads the config file and sets up the necessary variables for making the plots.

Here's an example of how to use pyfragplotter:
