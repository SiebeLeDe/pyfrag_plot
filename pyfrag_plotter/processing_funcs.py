...
def trim_data(df: pd.DataFrame, trim_option: Optional[Union[str, float, int, Sequence]] = None, trim_key: Optional[str] = None) -> pd.DataFrame:
    """'Overloaded' function to trim (shorten) the dataframe based on the type of the trim_option.

    This function trims the given dataframe based on the type of the trim_option.
    The trim_option can be either a string ("min", "max"), integer (IRC point), float (energy value), or a sequence (x_limits such as (1.0, 3.0))). If none provided, it is read from the config file.
    ...
    Args:
        ...
        trim_key (Optional[str]): The key to use for reading the trim_option. Defaults to None.
